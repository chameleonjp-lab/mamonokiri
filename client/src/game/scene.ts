...
function mat(scene: Scene, name: string, color: Color3, emissive = 0) {
  const m = new StandardMaterial(name, scene); m.diffuseColor = color; m.specularColor = new Color3(0.08, 0.08, 0.08); m.emissiveColor = color.scale(emissive); return m;
}
function box(scene: Scene, name: string, size: Vector3, pos: Vector3, material: StandardMaterial) {
  const m = MeshBuilder.CreateBox(name, { width: size.x, height: size.y, depth: size.z }, scene); m.position = pos; m.material = material; return m;
}
function makePlayer(scene: Scene, materials: Record<string, StandardMaterial>) {
  const root = new Mesh("yamabushi_samurai", scene); root.position = new Vector3(0, 0.18, 0);
  const torso = box(scene, "samurai_kimono", new Vector3(0.82, 0.92, 0.52), new Vector3(0, 1.15, 0), materials.indigo); torso.parent = root;
  const lapel = box(scene, "kimono_lapel", new Vector3(0.16, 0.72, 0.56), new Vector3(0.08, 1.25, -0.03), materials.cream); lapel.rotation.z = -0.16; lapel.parent = root;
  const sash = box(scene, "obi_sash", new Vector3(0.88, 0.16, 0.58), new Vector3(0, 0.82, -0.01), materials.leather); sash.parent = root;
  const knot = box(scene, "obi_knot", new Vector3(0.22, 0.24, 0.12), new Vector3(-0.38, 0.86, 0.24), materials.vermilion); knot.parent = root;
  const hakama = MeshBuilder.CreateCylinder("hakama", { height: 0.58, diameterTop: 0.72, diameterBottom: 1.0, tessellation: 6 }, scene); hakama.position = new Vector3(0, 0.46, 0); hakama.material = materials.indigo; hakama.parent = root;
  const leftLeg = box(scene, "left_leg", new Vector3(0.2, 0.54, 0.26), new Vector3(-0.2, 0.12, 0), materials.gaiter); leftLeg.parent = root;
  const rightLeg = box(scene, "right_leg", new Vector3(0.2, 0.54, 0.26), new Vector3(0.2, 0.12, 0), materials.gaiter); rightLeg.parent = root;
  const leftSandal = box(scene, "left_zori", new Vector3(0.3, 0.08, 0.48), new Vector3(-0.2, -0.16, -0.03), materials.wood); leftSandal.parent = root;
  const rightSandal = box(scene, "right_zori", new Vector3(0.3, 0.08, 0.48), new Vector3(0.2, -0.16, -0.03), materials.wood); rightSandal.parent = root;
  const head = MeshBuilder.CreateSphere("samurai_face", { diameter: 0.48, segments: 12 }, scene); head.position = new Vector3(0, 1.9, -0.02); head.material = materials.skin; head.parent = root;
  const hair = MeshBuilder.CreateSphere("chonmage_hair", { diameter: 0.52, segments: 8 }, scene); hair.scaling = new Vector3(0.92, 0.58, 0.92); hair.position = new Vector3(0, 2.13, 0.02); hair.material = materials.hair; hair.parent = root;
  const topknot = MeshBuilder.CreateCylinder("topknot", { height: 0.22, diameterTop: 0.14, diameterBottom: 0.22, tessellation: 8 }, scene); topknot.position = new Vector3(0, 2.32, 0.02); topknot.material = materials.hair; topknot.parent = root;
  const headband = box(scene, "hachimaki", new Vector3(0.5, 0.06, 0.5), new Vector3(0, 2.0, -0.02), materials.vermilion); headband.parent = root;
  const neckCloth = box(scene, "neck_cloth", new Vector3(0.42, 0.16, 0.42), new Vector3(0, 1.65, 0), materials.cream); neckCloth.parent = root;
  const leftArm = box(scene, "left_sleeve", new Vector3(0.2, 0.72, 0.24), new Vector3(-0.52, 1.15, 0), materials.indigo); leftArm.rotation.z = 0.16; leftArm.parent = root;
  const rightArm = box(scene, "right_sleeve", new Vector3(0.2, 0.72, 0.24), new Vector3(0.52, 1.15, 0), materials.indigo); rightArm.rotation.z = -0.16; rightArm.parent = root;
  const staff = MeshBuilder.CreateCylinder("staff", { height: 1.9, diameter: 0.07, tessellation: 8 }, scene); staff.position = new Vector3(-0.72, 1.05, 0.05); staff.rotation.z = -0.1; staff.material = materials.wood; staff.parent = root;
  const scabbard = box(scene, "katana_scabbard", new Vector3(0.1, 1.18, 0.1), new Vector3(0.28, 0.82, 0.22), materials.wood); scabbard.rotation.z = -0.72; scabbard.parent = root;
  const blade = box(scene, "katana", new Vector3(0.07, 1.45, 0.08), new Vector3(0.68, 1.3, -0.08), materials.steel); blade.rotation.z = -0.65; blade.parent = root;
  const guard = MeshBuilder.CreateTorus("tsuba", { diameter: 0.2, thickness: 0.035, tessellation: 16 }, scene); guard.position = new Vector3(0.4, 1.73, -0.08); guard.rotation.y = Math.PI / 2; guard.material = materials.gold; guard.parent = root;
  const prayer = MeshBuilder.CreateTorus("prayer_beads", { diameter: 0.28, thickness: 0.025, tessellation: 12 }, scene); prayer.position = new Vector3(-0.2, 1.62, -0.28); prayer.rotation.x = Math.PI / 2; prayer.material = materials.vermilion; prayer.parent = root;
  return { root, blade, torso, leftArm, rightArm, leftLeg, rightLeg, head, hair, neckCloth, prayer, sash, scabbard };
}
type EnemyAttackSide = "left" | "right" | "alternate" | "wide" | "target";
type EnemyVariant = { name: string; epithet: string; family: string; shape: number; attackSide: EnemyAttackSide; cooldown: number; notice: string; boss?: boolean };
const ENEMY_VARIANTS: EnemyVariant[] = [
  { name: "影面", epithet: "左薙の影", family: "通常", shape: 0, attackSide: "left", cooldown: 3000, notice: "左槍が赤く灯る。右へ避けよ。" },
  { name: "角岩", epithet: "右穿の角", family: "通常", shape: 1, attackSide: "right", cooldown: 3400, notice: "右槍が赤く灯る。左へ避けよ。" },
  { name: "鳴壺", epithet: "交互の壺", family: "通常", shape: 2, attackSide: "alternate", cooldown: 2700, notice: "槍が交互に鳴る。構えを読め。" },
  { name: "逆鉾", epithet: "長柄の異形", family: "通常", shape: 3, attackSide: "target", cooldown: 3600, notice: "矛先がこちらを追う。横へ流せ。" },
  { name: "霧猿", epithet: "跳ねる双影", family: "通常", shape: 4, attackSide: "wide", cooldown: 4100, notice: "左右の槍が開く。中央を離れよ。" },
  { name: "鉄輪", epithet: "重圧の輪", family: "通常", shape: 5, attackSide: "target", cooldown: 4500, notice: "重い一突き。予備動作を見よ。" },
  { name: "骨灯", epithet: "惑いの灯", family: "通常", shape: 6, attackSide: "alternate", cooldown: 2350, notice: "灯が明滅する。次の槍を読め。" },
];
const BOSS_VARIANTS: EnemyVariant[] = [
  { name: "牙嶺", epithet: "月喰らいの獣", family: "獣型", shape: 4, attackSide: "target", cooldown: 3200, notice: "牙嶺が低く沈む。防御を構えよ。", boss: true },
  { name: "鎧熊", epithet: "鉄皮の荒王", family: "獣型", shape: 5, attackSide: "wide", cooldown: 4200, notice: "鎧熊の双槍。両腕を固めよ。", boss: true },
  { name: "沼喰", epithet: "深淵の異形", family: "モンスター型", shape: 0, attackSide: "alternate", cooldown: 2600, notice: "沼喰の槍が脈打つ。次を読め。", boss: true },
  { name: "百眼", epithet: "千穴の怪", family: "モンスター型", shape: 1, attackSide: "target", cooldown: 3000, notice: "百眼が矛先を追わせる。防御せよ。", boss: true },
  { name: "鬼将", epithet: "赤面の武門", family: "人型", shape: 3, attackSide: "left", cooldown: 2900, notice: "鬼将の左薙。右へ流れよ。", boss: true },
  { name: "白面", epithet: "無音の剣客", family: "人型", shape: 2, attackSide: "right", cooldown: 2500, notice: "白面の右穿。左へ退け。", boss: true },
  { name: "天狗鴉", epithet: "空駆ける嘴", family: "鳥型", shape: 6, attackSide: "alternate", cooldown: 2200, notice: "天狗鴉が翼を開く。交互に防げ。", boss: true },
  { name: "鶴骸", epithet: "白羽の亡霊", family: "鳥型", shape: 4, attackSide: "target", cooldown: 2800, notice: "鶴骸の急降下。防御を構えよ。", boss: true },
  { name: "石門王", epithet: "割れ鳥居の主", family: "モニュメント型", shape: 5, attackSide: "wide", cooldown: 4400, notice: "石門王、両槍を開く。受け止めよ。", boss: true },
  { name: "巨仏", epithet: "沈黙する座像", family: "モニュメント型", shape: 0, attackSide: "wide", cooldown: 5000, notice: "巨仏の両槍。回避せず防御せよ。", boss: true },
];

function makeEnemy(scene: Scene, materials: Record<string, StandardMaterial>, variant: EnemyVariant) {
  const root = new Mesh("mountain_wraith", scene); root.position = new Vector3(0, 0.2, 5.2);
  const body0 = MeshBuilder.CreatePolyhedron("wraith_body_shadow", { type: 1, size: 1.45 }, scene); body0.position.y = 1.05; body0.scaling.y = 1.4; body0.material = materials.stone; body0.parent = root;
  const body1 = MeshBuilder.CreateCylinder("wraith_body_rock", { height: 1.8, diameterTop: 0.72, diameterBottom: 1.28, tessellation: 6 }, scene); body1.position.y = 1.05; body1.material = materials.stone; body1.parent = root;
  const body2 = MeshBuilder.CreateSphere("wraith_body_vessel", { diameter: 1.35, segments: 8 }, scene); body2.position.y = 1.05; body2.scaling.y = 1.18; body2.material = materials.stone; body2.parent = root;
  const body3 = MeshBuilder.CreateBox("wraith_body_spear", { width: 1.1, height: 1.9, depth: 0.9 }, scene); body3.position.y = 1.05; body3.rotation.z = 0.08; body3.material = materials.stone; body3.parent = root;
  const body4 = MeshBuilder.CreateTorus("wraith_body_mist", { diameter: 1.5, thickness: 0.38, tessellation: 8 }, scene); body4.position.y = 1.05; body4.rotation.x = Math.PI / 2; body4.material = materials.stone; body4.parent = root;
  const body5 = MeshBuilder.CreateCylinder("wraith_body_ring", { height: 1.35, diameterTop: 1.38, diameterBottom: 0.86, tessellation: 8 }, scene); body5.position.y = 1.05; body5.material = materials.stone; body5.parent = root;
  const body6 = MeshBuilder.CreatePolyhedron("wraith_body_lantern", { type: 2, size: 1.45 }, scene); body6.position.y = 1.05; body6.scaling.y = 1.2; body6.material = materials.stone; body6.parent = root;
  const bodies = [body0, body1, body2, body3, body4, body5, body6]; bodies.forEach((body, index) => { body.isVisible = index === variant.shape; });
  const body = bodies[variant.shape];
  const eyeL = MeshBuilder.CreateSphere("eye_l", { diameter: 0.1, segments: 6 }, scene); eyeL.position = new Vector3(-0.22, 1.35, -0.6); eyeL.material = materials.amber; eyeL.parent = root;
  const eyeR = eyeL.clone("eye_r")!; eyeR.position.x = 0.22; eyeR.parent = root;
  const blade = box(scene, "wraith_blade", new Vector3(0.12, 1.25, 0.12), new Vector3(0.68, 1.05, -0.1), materials.iron); blade.rotation.z = -0.7; blade.parent = root;
  const spearL = box(scene, "wraith_spear_l", new Vector3(0.09, 0.09, 1.15), new Vector3(-0.72, 1.08, -0.62), materials.iron); spearL.parent = root;
  const spearR = box(scene, "wraith_spear_r", new Vector3(0.09, 0.09, 1.15), new Vector3(0.72, 1.08, -0.62), materials.iron); spearR.parent = root;
  const spearTipL = MeshBuilder.CreateCylinder("wraith_spear_tip_l", { height: 0.24, diameterTop: 0, diameterBottom: 0.18, tessellation: 4 }, scene); spearTipL.rotation.x = Math.PI / 2; spearTipL.position = new Vector3(-0.72, 1.08, -1.28); spearTipL.material = materials.iron; spearTipL.parent = root;
  const spearTipR = spearTipL.clone("wraith_spear_tip_r")!; spearTipR.position.x = 0.72; spearTipR.parent = root;
  return { root, body, bodies, eyes: [eyeL, eyeR], blade, spears: [spearL, spearR], spearTips: [spearTipL, spearTipR] };
}
function announce(state: State) { window.dispatchEvent(new CustomEvent("yamabushi-state", { detail: state })); }

export async function createGameScene(engine: Engine, canvas: HTMLCanvasElement): Promise<GameHandle> {
  const scene = new Scene(engine); scene.clearColor = new Color4(0.06, 0.075, 0.09, 1); scene.fogMode = Scene.FOGMODE_EXP2; scene.fogDensity = 0.045; scene.fogColor = MIST;
  const camera = new FreeCamera("camera", new Vector3(7.6, 5.2, -8.5), scene); camera.setTarget(new Vector3(0, 1.05, 2.2)); camera.attachControl(canvas, true); camera.speed = 0.15; camera.minZ = 0.1;
  const light = new HemisphericLight("moonlight", new Vector3(-0.2, 1, -0.4), scene); light.intensity = 0.72; light.diffuse = new Color3(0.62, 0.7, 0.78); light.groundColor = new Color3(0.05, 0.06, 0.07);
  const materials = { ink: mat(scene, "ink", INK), enemyGlow: mat(scene, "enemy_glow", new Color3(0.92, 0.48, 0.62), 0.42), bossGlow: mat(scene, "boss_glow", new Color3(0.62, 0.04, 0.22), 0.72), spearReady: mat(scene, "spear_ready", VERMILION, 0.95), indigo: mat(scene, "indigo", new Color3(0.09, 0.12, 0.18)), skin: mat(scene, "skin", new Color3(0.46, 0.31, 0.23)), gaiter: mat(scene, "gaiter", new Color3(0.75, 0.73, 0.65)), wood: mat(scene, "wood", new Color3(0.28, 0.16, 0.1)), steel: mat(scene, "steel", new Color3(0.68, 0.73, 0.75), 0.16), vermilion: mat(scene, "vermilion", VERMILION, 0.28), stone: mat(scene, "stone", new Color3(0.12, 0.14, 0.16)), amber: mat(scene, "amber", new Color3(0.95, 0.45, 0.12), 0.8), iron: mat(scene, "iron", new Color3(0.22, 0.24, 0.25)), cream: mat(scene, "cream", new Color3(0.82, 0.76, 0.64)), leather: mat(scene, "leather", new Color3(0.22, 0.09, 0.05)), hair: mat(scene, "hair", new Color3(0.025, 0.02, 0.018)), gold: mat(scene, "gold", new Color3(0.72, 0.45, 0.13), 0.14) };
